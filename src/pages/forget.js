...
class ForgotPassword extends React.Component{
    ...
    onSubmit =(event)=>{
        ...
        if(this.state.username === '')
            ...
            .then((res)=>{
                if(res.code === 200)
                this.setState({loading:false, type:true, err:'An email has been sent to your email address'})
                else
                this.setState({loading:false, type:false, err:res.message})
            })
            .catch((err)=>{
                this.setState({
                    loading:false, 
                        ...
                        <div className="form">
                            {
                                ...
                                >
                                    <Alert variant="filled" severity={!this.state.type?'error':'success'}>{this.state.err}</Alert>
                                </Snackbar>
                            }
                        {/* <p className="error">{this.state.err}</p> */}
                            <form onSubmit={event=>{
                                ...
                                <div className="form-check">
                                    <p><Link to="/register">Return to Sign in</Link></p>
